defmodule VideoTool.Settings do
  @moduledoc """
  자격증명과 환경 상태.
  ...
  저장은 항상 DPAPI(`VideoTool.Credentials`)로 한다. DB 에도 `.env` 에도 평문으로 두지 않는다.
  """

  alias VideoTool.{Credentials, Flow, Ocr, Ffmpeg}
  ...
  @doc """
  값이 어디서 왔는지. 화면에서 넣은 값이 `.env` 를 이기므로,
  `.env` 를 고쳤는데 안 바뀌면 이유를 여기서 알 수 있어야 한다.
  """
  def source(name) do
    case definition(name) do
      nil ->
      ...
      {_key, ref, env, _label, _help} ->
        cond do
          match?({:ok, v} when v != "", Credentials.get(ref)) -> "화면"
          blank_to_nil(System.get_env(env)) -> ".env"
          true -> nil
        end
    end
  end
