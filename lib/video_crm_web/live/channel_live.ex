defmodule VideoCRMWeb.ChannelLive do
  @moduledoc """
  발행 채널. 유튜브·인스타 계정을 연결하고, 어디로 올릴지 고른다.

  실제 토큰은 여기 없다 — `VideoCRM.Credentials` 가 Windows DPAPI 로 암호화해 파일에 둔다.
  ...
  alias VideoCRM.{Credentials, Media, Projects, Publishing}
  ...
  def handle_event("publish", %{"publication_id" => id}, socket) do
    with {:ok, publication} <- Publishing.get_publication(id),
      ...
      # publish/4 는 업로드가 붙기 전까지 {:error, _} 만 돌려준다.
      # 성공 절은 업로드를 구현할 때 여기 추가한다 — 미리 넣어두면 안 돌아가는 코드가 남는다.
