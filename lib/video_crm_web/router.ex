defmodule VideoCRMWeb.Router do
  use VideoCRMWeb, :router

  pipeline :browser do
    plug :accepts, ["html"]
    plug :fetch_session
    plug :fetch_live_flash
    plug :put_root_layout, html: {VideoCRMWeb.Layouts, :root}
  ...
  scope "/", VideoCRMWeb do
    pipe_through :browser

    # 루트가 곧 컨택트시트 목록이다. Phoenix 환영 페이지는 남겨둘 이유가 없다 —
    # 실제로 루트를 열고 "화면이 그대로다" 라고 오해하는 일이 있었다.
    live "/", ProjectLive.Index, :index
    live "/projects", ProjectLive.Index, :index
    live "/projects/:id", ProjectLive.Show, :show

    # 프롬프트 편집 — 바꿔 끼울 칸(변수)과 본문을 여기서 고친다.
    live "/dashboard", DashboardLive, :index
    live "/series", SeriesLive, :index
    live "/prompts", PromptLive, :index

    # 발행 채널 — 연결 상태와 발행 대기 목록.
  ...
  if Application.compile_env(:video_crm, :dev_routes) do
    ...
    scope "/dev" do
      ...
      live_dashboard "/dashboard", metrics: VideoCRMWeb.Telemetry
