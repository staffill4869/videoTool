defmodule VideoCRM.Pipeline do
  @moduledoc """
  ...
  alias VideoCRM.{Ingest, Jobs, Media, Projects, Prompt, Publishing}

  # 테스트에서 실제 클립보드를 건드리지 않도록 갈아끼운다.
  defp clipboard, do: Application.get_env(:video_crm, :clipboard, VideoCRM.Clipboard)

  # 테스트가 브라우저 유무에 좌우되면 안 된다.
  defp flow, do: Application.get_env(:video_crm, :flow, VideoCRM.Flow)
  ...
  defp stage_step(project, stage, count, present, mapped) do
    ...
    cond do
      ...
      # 파일은 들어왔는데 매핑이 덜 됐다 — 사람이 확인해야 한다
      present[kind] > 0 and mapped[kind] < count ->
        %{
          stage: stage,
          action: "agent",
          message:
            "#{kind} 파일 #{present[kind]}개 중 #{mapped[kind]}개만 장면에 붙었습니다. " <>
              "빠진 컷을 다시 생성하거나, 파일을 직접 지정해 ingest(path:) 를 부르세요.",
    ...
    cond do
      is_nil(narration) ->
        %{
          ...
          action: "blocked",
          message:
            "클립이 모두 준비됐습니다. 나레이션 생성(generate_narration)은 아직 구현되지 " <>
              "않았습니다 (설명서 4주차 — 힉스필드 TTS + 무음 정렬 + 자막).",
          scenes_ready: true
        }
      ...
      is_nil(render) ->
        %{
          stage: "assemble",
          action: "blocked",
          message: "나레이션이 준비됐습니다. 합성(assemble)은 아직 구현되지 않았습니다 (설명서 4주차).",
