defmodule VideoCRM.Publishing.GoogleOAuth do
  @moduledoc """
  ...
  **state 는 DB 없이 `Phoenix.Token` 서명으로 처리한다** (10분 유효).
  ...
  alias VideoCRM.{Credentials, Publishing, Settings}
  ...
  def redirect_uri do
    Application.get_env(:video_crm, :google_redirect_uri) ||
  ...
  def authorize_url(channel_slug) do
    if configured?() do
      state = Phoenix.Token.sign(VideoCRMWeb.Endpoint, @state_salt, channel_slug)
  ...
  defp verify_state(state) do
    case Phoenix.Token.verify(VideoCRMWeb.Endpoint, @state_salt, state, max_age: @state_max_age) do
      {:ok, slug} -> {:ok, slug}
