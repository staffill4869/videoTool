defmodule VideoCRM.Projects.Project do
  @moduledoc false
  use Ecto.Schema
...
defmodule VideoCRM.Projects.Script do
  @moduledoc false
  use Ecto.Schema
  import Ecto.Changeset
...
defmodule VideoCRM.Projects.Scene do
  @moduledoc """
...
defmodule VideoCRM.Projects.ScriptSegment do
  @moduledoc "대본 구간만 버전별로 갈아끼운다. Scene 은 그대로 둔다."
  use Ecto.Schema
  import Ecto.Changeset

  alias VideoCRM.Projects.{Script, Scene}
...
defmodule VideoCRM.Projects.AllowedFact do
  @moduledoc """
  ...
  alias VideoCRM.Projects.Script
...
defmodule VideoCRM.Projects do
  @moduledoc "프로젝트 · 대본 · 장면. 에이전트가 만든 결과물을 받아 저장한다."

  import Ecto.Query
  alias Ecto.Multi
  alias VideoCRM.{Presets, Repo}
  alias VideoCRM.Projects.{Project, Script, Scene, ScriptSegment, AllowedFact}
  ...
  def create_project(attrs) do
    with {:ok, style} <- Presets.fetch_style(attrs["style_slug"]),
      ...
      params =
        attrs
        |> Map.take(["title", "topic", "target_sec", "aspect", "output_folder", "pipeline", "variables"])
        |> Map.merge(%{
          "style_id" => style.id,
          "domain_id" => domain.id,
          "voice_id" => voice.id,
  ...
  def work_root do
    Application.get_env(:video_crm, :work_root) || Path.join(File.cwd!(), "projects")
  end
  ...
  **CLEAN 이미지를 다시 만들지 않는다.** CLEAN 에는 글자가 없으므로 언어가 달라도 그대로 쓴다.
  ...
  defp copy_clean_assets(source, variant) do
    ...
    entries =
      from(a in VideoCRM.Media.Asset,
    ...
    {count, _} = Repo.insert_all(VideoCRM.Media.Asset, entries)
    count
  end
