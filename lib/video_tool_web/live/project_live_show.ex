defmodule VideoToolWeb.ProjectLive.Show do
  @moduledoc """
  ...
  alias VideoTool.{Jobs, Mapping, Media, Pipeline, Projects, Prompt, Validation}
  ...
  def handle_event("set_subtitle_font", %{"subtitle_font" => font}, socket) do
    case Projects.update_project(socket.assigns.project, %{"subtitle_font" => font}) do
      {:ok, _} ->
        label = if font == "", do: "기본", else: font
        {:noreply, socket |> put_flash(:info, "자막 폰트: #{label}") |> reload()}

      {:error, _} ->
        {:noreply, put_flash(socket, :error, "폰트를 저장하지 못했습니다")}
    end
  end
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} active={:projects}>
      ...
      <div class="mt-4 flex flex-wrap gap-2 text-sm">
        ...
        <span :if={@narration} class="badge badge-lg badge-ghost">
      ...
      <section class="mt-6">
        ...
        <form phx-change="set_subtitle_font" class="mb-3 flex items-center gap-2 text-sm">
          <span class="opacity-70">자막 폰트</span>
          <select name="subtitle_font" class="select select-bordered select-sm">
            <option value="" selected={@project.subtitle_font in [nil, ""]}>
              기본 ({VideoTool.Presets.default_subtitle_font()})
            </option>
            <option
              :for={{label, name} <- VideoTool.Presets.subtitle_fonts()}
              value={name}
              selected={name == @project.subtitle_font}
            >
              {label}
            </option>
          </select>
          <%!-- 이미 구운 영상은 안 바뀐다. 폰트는 굽는 시점에 박히기 때문이다. --%>
          <span class="text-xs opacity-50">바꾸면 다음 합성부터 적용됩니다</span>
        </form>

        <p :if={@renders == []} class="text-sm opacity-60">
          아직 완성본이 없습니다. 클립과 나레이션이 준비되면 assemble 로 만듭니다.
        </p>
          ...
          <div :for={r <- @renders} class="card bg-base-200">
            <div class="card-body gap-2 p-3">
              ...
              <div class="flex flex-wrap items-center gap-2 text-xs opacity-70">
                <span class="badge badge-sm">{r.aspect}</span>
                <span>{Float.round(r.duration_sec, 1)}초</span>
                <span>{mb(r.file_size)} MB</span>
                <span :if={r.burn_subtitles} class="badge badge-ghost badge-sm">자막 있음</span>
                <span>{Calendar.strftime(r.inserted_at, "%m/%d %H:%M")}</span>
              </div>
              <div class="flex gap-3 text-xs">
                <a href={~p"/renders/#{r.id}/play"} download class="link">내려받기</a>
                <%!-- 자막이 화면을 가리는지 볼 때 쓴다. 세로본·다른 언어판이 쓰는 것도 이 파일이다. --%>
                <a
                  :if={r.settings["master_no_subs"]}
                  href={~p"/renders/#{r.id}/play?variant=master"}
                  target="_blank"
                  class="link"
                >
                  자막 없는 마스터
        ...
        <div>
          <div class="mb-1 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
              ...
              <button
                phx-click={JS.dispatch("videotool:copy", detail: %{text: @prompt_rendered})}
  ...
  defp mb(bytes) when is_integer(bytes), do: Float.round(bytes / 1_048_576, 1)
  defp mb(_), do: 0.0
end