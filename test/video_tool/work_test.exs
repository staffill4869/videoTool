defmodule VideoTool.WorkTest do
  @moduledoc """
  작업 큐 · 시리즈 · 성과 집계.
  ...
  alias VideoTool.{Insights, Media, Projects, Publishing, Series, Work}

  setup do
    Code.eval_file("priv/repo/seeds.exs")
    {:ok, style} = VideoTool.Presets.fetch_style("iso-lowpoly")
    {:ok, domain} = VideoTool.Presets.fetch_domain("history-military")
    {:ok, voice} = VideoTool.Presets.fetch_voice("mark")
  ...
  describe "언어판" do
    ...
    test "표기언어 변수가 프로젝트 언어에서 자동으로 채워진다" do
      ...
      assert VideoTool.Prompt.variables(variant)["표기언어"] == "영어(English)"
      assert VideoTool.Prompt.variables(source)["표기언어"] == "한국어"
    end
  end
  ...
  describe "프로젝트별 프롬프트" do
    test "전용 프롬프트가 있으면 공용 템플릿 대신 쓴다" do
      ...
      refute VideoTool.Prompt.overridden?(project, "clean")

      {:ok, project} = Projects.set_prompt_override(project, "clean", "이 프로젝트만의 프롬프트")
      {:ok, project} = Projects.get_project(project.id)

      assert VideoTool.Prompt.overridden?(project, "clean")
      {:ok, text} = VideoTool.Prompt.render(project, "clean")
