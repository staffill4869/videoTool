defmodule VideoCRM.IngestTest do
  @moduledoc """
  ...
  alias VideoCRM.{Ffmpeg, Ingest, Media, Projects}
  ...
  setup do
    ...
    prev = Application.get_env(:video_crm, :downloads_dir)
    Application.put_env(:video_crm, :downloads_dir, downloads)
    ...
    on_exit(fn ->
      Application.put_env(:video_crm, :downloads_dir, prev)
      File.rm_rf(tmp)
  ...
  test "INFO 는 제 CLEAN 에 가장 가깝다 (2등과 뚜렷이 벌어진다)", ctx do
    ...
    for info <- infos do
      ...
      mine = VideoCRM.Phash.similarity(own.phash, info.phash)
      best_other = others |> Enum.map(&VideoCRM.Phash.similarity(&1.phash, info.phash)) |> Enum.max()
