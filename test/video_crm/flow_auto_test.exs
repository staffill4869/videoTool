defmodule VideoCRM.FlowAutoTest do
  @moduledoc """
  Flow 브라우저 자동 조종의 분기.
  ...
  alias VideoCRM.{Jobs, Pipeline, Projects}
  ...
  setup do
    ...
    prev = Application.get_env(:video_crm, :flow_stub_status)
    on_exit(fn ->
      Application.put_env(:video_crm, :flow_stub_status, prev)
  ...
  test "자동이 켜져 있고 준비됐으면 생성을 걸고 기다리라고 답한다", %{project: project} do
    Application.put_env(:video_crm, :flow_stub_status, {:ok, %{flow_tab: true, prompt_box: true}})
  ...
  test "Chrome 이 없으면 멈추지 않고 수동으로 되돌아간다", %{project: project} do
    Application.put_env(:video_crm, :flow_stub_status, {:error, "Chrome 에 붙지 못했습니다"})
    project = as_auto(project)
